...
// To check if PMD has been initialized
extern unsigned char pmd_init;
...
static void PMD_Init_PPS()
{
    ...
    // Set AH to 0x18 for setting PPSDRV
    inregs.h.ah = 0x18;
...
static void pmd_play_pps_sound_effect(unsigned char sound_effect_index)
{
    ...
    // Set AH to 0x03 for PCM PSG
    inregs.h.ah = 0x03;

    // Set AL to the sound effect index
    inregs.h.al = 128 + sound_effect_index;
...
static unsigned char use_pps;
static unsigned char use_p86;
static unsigned short frequency;
static unsigned char use_ppc;

unsigned char PMDPCM_Init()
{
	use_p86 = 0;
	use_pps = 0;
	frequency = 2540+16000;
	use_ppc = 3;
	
	// PMD not initialized error !!!
	if (pmd_init == 0) return 1;
	
	PMD_Init_PPS();
	use_pps = PMD_Check_PPS();
...
{	
	// PMD not initialized error !!!
	if (pmd_init == 0 || use_p86 == 0) return 1;
	
	// Assume P86 from now on
	// Use 0 instead of 3 for channel (3 is PPC specific)
	frequency = 0x2200;
	use_ppc = 0;
...
void PMDPCM_Play(struct WaveData *vc, unsigned char index)
{
	...
	pmd_play_pcm_sound_effect(index, frequency, use_ppc, 255);
	if (use_pps) pmd_play_pps_sound_effect(index);
