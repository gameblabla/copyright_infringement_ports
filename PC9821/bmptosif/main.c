...
int main (int argc, char *argv[]) 
{
	...
	else
	{
		...
		else if (strstr (argv[1],".lzsa2"))
		{
			printf("Custom binary format is LZSA2\n");
			encoding = 5;
		}
	...
	if (noheader == 0)
	{
		...
		fwrite (&encoding, sizeof(char), sizeof(encoding), fp);
		
		// These stubs were original required for the Sharp X68000 but we'll reuse them. One is already used for endianness.
		fwrite (&endianess, sizeof(char), sizeof(endianess), fp);
		printf("Filesize is %d\n", lSize);
		fwrite (&lSize, sizeof(char), sizeof(lSize), fp);
