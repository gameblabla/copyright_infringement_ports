/****************************************************************************
** Demonstration of playing a single wave file through DMA using __DJGPP__     **
**  by Steven H Don                                                        **
**                                                                         **
...
{
	outp(0x20, 0x20); // IRQ ack, unlike our main interrupts, this is required or else no keyboard input upon exit
}
#endif
...
static void ClearBuffer (unsigned int Buffer)
{
	...
	dosmemput(Address, BLOCK_SIZE, DOSBufOfs + (Buffer << 13));
#else
	//Fill an 8K block in the DMA buffer with 128's - silence
...
static void MixVoice (struct WaveData *Voice)
{
  ...
  */
  ...
  if (OffSet > beforeOffSet)
    ...
    dosmemput (DMABuffer + beforeOffSet, BUFFER_SIZE - beforeOffSet, DOSBufOfs + beforeOffSet);
...
** checking.                                                               **
****************************************************************************/
static unsigned char LoadVoice (struct WaveData *Voice, const char *FileName)
{
    int WAVFile;
    unsigned ii;
    unsigned fileLength;
    ...
    Voice->SoundLength = fileLength - 48;
    lseek(WAVFile, 0L, SEEK_SET);

    Voice->Sample = (unsigned char *)malloc(Voice->SoundLength + 2); // Assign memory
    if (!Voice->Sample) return 1;
    
    memset(Voice->Sample, 0x00, Voice->SoundLength + 2);
    
    lseek(WAVFile, 46L, SEEK_SET);                                  // Skip the header

    // Load the sample data
    _dos_read(WAVFile, Voice->Sample, Voice->SoundLength, &ii);
...
static void AssignBuffer ()
{
  ...
  if (!DMABuffer) DMABuffer = (unsigned char *)malloc (BUFFER_SIZE);
  ...
  dosmemput (DMABuffer, BUFFER_SIZE, DOSBufOfs);
#else
...
static unsigned char SBDMA_Load(const char* WaveFile, struct WaveData *mvc, unsigned char index)
{
	return LoadVoice (mvc, WaveFile);
}

static void SBDMA_Play(struct WaveData *vc, unsigned char index)
{
...
static void SBDMA_Unload(struct WaveData *vc, unsigned char index)
{
	if (vc->Sample)
	{
		free(vc->Sample);
