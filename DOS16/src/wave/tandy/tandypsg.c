...
static unsigned char Tandy_Init()
{
	...
	SetPCjrAudioPeriod(1, 0x000F);
	SetPCjrAudioPeriod(2, 0x000F);
	SetPCjrAudioVolume(0,0xF);
...
static unsigned char Tandy_Load(const char* WaveFile, struct WaveData *vc, unsigned char index)
{
	return Sound_Load_File(WaveFile, vc, index);
}

static void Tandy_Play(struct WaveData *vc, unsigned char index)
{
	uint32_t frequency;
	
	if (tandy_play == 1 || tandy_init == 0) return;
...
static void Tandy_Unload(struct WaveData *vc, unsigned char index)
{
	if (vc->Sample)
	{
		free(vc->Sample);
