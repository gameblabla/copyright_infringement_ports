...
void Generic_Load_SIF(const char *file, BITMAP *b, unsigned short s_width, unsigned short s_height, unsigned char dontdecompress)
{
    int fd;
    char str[16];
...
unsigned char Sound_Load_File(const char* WaveFile, struct WaveData *vc, unsigned char index)
{
    int WAVFile;
    unsigned ii;
    unsigned fileLength;
	...
	vc->SoundLength = lseek(WAVFile, 0L, SEEK_END);
    lseek(WAVFile, 0L, SEEK_SET);

    vc->Sample = (unsigned char *)malloc(vc->SoundLength); // Assign memory
    if (!vc->Sample) return 1;
    memset(vc->Sample, 0x00, vc->SoundLength);

    // Load the sample data
    _dos_read(WAVFile, vc->Sample, vc->SoundLength, &ii);
