...
#if defined(__386__)
static unsigned char *HGC = (unsigned char*)0xB0000;
#else
static unsigned char __far *HGC = (unsigned char __far*)0xB0000000;
#endif
...
static void drawChar(char ch, unsigned short x, unsigned short y, unsigned char color_index)
{
    ...
    unsigned char pixel_mask;
    unsigned short y_div_4, y_mod_4;
    ...
    while (i--)
    {
        ...
        y_mod_4 <<= 13;
        j = 8;
        offset = (y_div_4 << 6) + (y_div_4 << 4) + (y_div_4 << 3) + (y_div_4 << 1) + y_mod_4;
        while (j--)
        {
            plot_x = x + (7 - j);
            offset2 = offset + (plot_x >> 3);
            pixel_mask = 0x80 >> (plot_x & 7);
            HGC[offset2] = (HGC[offset2] & ~pixel_mask) | (-((charSprite[i] >> j) & 1) & pixel_mask);
        }
    }
    x += 8;
}
...
{
	unsigned long off;
	unsigned short y_div_4 = y >> 2;
	unsigned short y_mod_4 = y & 3;
...
{
	HGC_Draw_Text(str, (screen_width - (strlen(str) << 3)) >> 1,  y, color_index);
}
