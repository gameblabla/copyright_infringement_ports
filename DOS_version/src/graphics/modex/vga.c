...
#else
static unsigned char __far *VGA = (unsigned char __far*)0xA0000000;
#endif
...
void plot_pixel_16(uint16_t x, uint16_t y, uint8_t color) {
    ...
    for (plane = 0; plane < 4; plane++) 
    {
		...
		VGA[base_offset] = VGA[base_offset] & ~mask;
		VGA[base_offset] = (VGA[base_offset] & ~mask) | (-((color >> plane) & 1) & mask);
    }
}
...
static unsigned char Init_VGA(unsigned short width, unsigned short height, unsigned short flags, int argc, char** argv)
{
	...
	// Doesn't work properly with palette for whatever reason
...
static void VGA_Draw_bitmap_compressed(BITMAP *bmp, short x, short y)
{
	...
	for(plane=0; plane<4; plane++)
	{
		outp(SC_DATA,  1 << ((plane+x)&3) );
			...
			CAL_LZSAExpand
#endif
		(bmp->data[plane],&VGA[((y*screen_width+x+plane) >> 2)]);
	}
}
