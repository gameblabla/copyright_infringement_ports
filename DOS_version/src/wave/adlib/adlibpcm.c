...
void (interrupt *old_isr)(); // Declare a global pointer to store the original ISR
...
#define ADLIB_DELAY_PAUSE() \
	...
	//inp(OPL2_PORT_REG); \ 6th delay, probably not needed
...
static unsigned char ADLIBPCM_Load(const char* WaveFile,  unsigned char index)
{
	// Cancel out audio
	ad_current_sample = 0;
	ad_sample_count = 1;
...
static void ADLIBPCM_Play( unsigned char index)
{

	if (adlib_pcm == 1) return;
	
	ad_samples = PCM_SoundData[index];
	ad_sample_count = PCM_SoundLength[index];
	ad_current_sample = 0;
	
	adlib_pcm = 1;
}

static void ADLIBPCM_Unload( unsigned char index)
{
