...
uint16_t pixel[] = { 0x8000, 0x4000, 0x2000, 0x1000, 0x0800, 0x0400, 0x0200, 0x0100,
...
void setPixel(uint16_t x, uint16_t y, uint16_t colour)
{
    uint32_t	ptr;
    uint16_t*	adr;
    uint16_t    mask = pixel[x & 0xF];

    ptr = (uint32_t)physbase + ((y << 7) + (y << 5)) + ((x >> 4) << 3);
    adr = (uint16_t*) ptr;
    
    *adr = (*adr & ~mask) | ((colour & 1) ? mask : 0);
    adr++;
    *adr = (*adr & ~mask) | ((colour & 2) ? mask : 0);
    adr++;
    *adr = (*adr & ~mask) | ((colour & 4) ? mask : 0);
    adr++;
    *adr = (*adr & ~mask) | ((colour & 8) ? mask : 0);
}


void drawChar(char ch, uint16_t x, uint16_t y, uint16_t colour)
{
	unsigned char i, j;
	...
	while (i--)
	{
		...
		while (j--)
		{
			if((charSprite[i] >> j) & 1)
			{
				setPixel(x + (7 - j), plot_y, colour);
			}
			else
			{
				setPixel(x + (7 - j), y + i, 0);
			}
		}
	}
	x += 8;
...
void Draw_Text(const char* str, unsigned short x, unsigned short y, unsigned short color_index)
{
	unsigned char i;
	for(i = 0; str[i]; i++)
...
void Draw_Text_Center(const char* str, unsigned short y, unsigned short color_index)
{
	unsigned char i;
	uint16_t x;
	uint16_t text_width;
